...
func (d *Driver) openDB(entity interface{}) ([]interface{}, error) {
	...
	if typeName[len(typeName)-1] == "Uploading" {
		if _, err := os.Stat(filepath.Join(d.dir, "Uploading")); os.IsNotExist(err) {
			fmt.Println("index dir created:", filepath.Join(d.dir, "Uploading"))
			if err = os.Mkdir(filepath.Join(d.dir, "Uploading"), 0755); err != nil {
				panic(fmt.Errorf("出错了, %v", err))
			}
		}
		file = filepath.Join(d.dir, "Uploading", entityName)
	} else if typeName[len(typeName)-1] == "Chunk" {
		if _, err := os.Stat(filepath.Join(d.dir, "Chunk")); os.IsNotExist(err) {
			if err = os.Mkdir(filepath.Join(d.dir, "Chunk"), 0755); err != nil {
				panic(fmt.Errorf("出错了, %v", err))
			}
		}
		file = filepath.Join(d.dir, "Chunk", entityName)
	} else {
		panic("Wrong type!")
	}
...
func (d *Driver) getEntityName() (string, error) {
	val := reflect.ValueOf(d.entityDealingWith)
	return filepath.Join(val.FieldByName("Upid").String()), nil
}
